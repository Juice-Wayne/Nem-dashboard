...
import { getWEMPrices, getWEMDemand, getWEMGeneration } from "@/lib/wem";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [prices, demand, generation] = await Promise.all([
      safe(getWEMPrices, "prices"),
      safe(getWEMDemand, "demand"),
      safe(getWEMGeneration, "generation"),
    ]);

    return NextResponse.json({ prices, demand, generation }, { headers });
  } catch (error) {
